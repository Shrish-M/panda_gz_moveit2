...
def build_tree(robot: RobotInterface) -> py_trees.behaviour.Behaviour:  # noqa: ARG001
    ...
    root.add_children([reset, grasp, place])

    return RepeatAlways(name='RepeatAlways', child=root)
...
class SelectObject(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        ...
        hx = container['width'] / 2.0
        hy = container['depth'] / 2.0

        for obj_id, obj in self.bb.detected_objects.items():
            x = obj.pose.position.x
            y = obj.pose.position.y
            in_container = abs(x - cx) < hx and abs(y - cy) < hy
            if not in_container:
                self.bb.target_object_id = obj_id
                self.robot.log(f'[SUCCESS]   SelectObject: selected {obj_id} for picking')
                return py_trees.common.Status.SUCCESS
...
class ProposeGrasps(py_trees.behaviour.Behaviour):
    ...
    def initialise(self):
        self._retries = 0
        self.bb.grasp_proposals = list()

    def update(self):
        obj_id = self.bb.target_object_id
        obj = self.bb.detected_objects.get(obj_id)
        if obj is None:
            self.robot.log(f'[FAIL] ProposeGrasps: target object {obj_id} not found')
            return py_trees.common.Status.FAILURE
        
        pos = obj.pose.position
        ori = obj.pose.orientation
        points = sample_cuboid_surface(
        ...
        candidates = detect_grasps(points)
        valid = []
        for candidate in candidates:
            
            approach_z = candidate.R[:, 0][2]
            above_table = candidate.pos[2] > self.bb.container['table_z'] + 0.01

            if approach_z < -0.5 and above_table:
                valid.append(gpd_to_panda_pose(candidate.pos, candidate.R))
        
        if not valid:
            if self._retries < self._MAX_RETRIES:
                self._retries += 1
                self.robot.log(f'[WARN] ProposeGrasps: no valid grasps, retrying ({self._retries}/{self._MAX_RETRIES})')
                return py_trees.common.Status.RUNNING
            else:
                self.robot.log('[FAIL] ProposeGrasps: no valid grasps after max retries')
                return py_trees.common.Status.FAILURE

        self.bb.grasp_proposals = valid
        self.robot.log(f'[SUCCESS] ProposeGrasps: found {len(valid)} valid grasp proposals')
        return py_trees.common.Status.SUCCESS
...
class ProposeDropPose(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        container = self.bb.container
        cx, cy = container['center_xy']
        table_z = container['table_z']
        height = container['height']

        drop_z = table_z + height + 0.01
        ...
        self.robot.log(f'[INFO] ProposeDropPose: dropping at ({cx:.2f}, {cy:.2f}, {drop_z:.2f})')
        return py_trees.common.Status.SUCCESS
